...
interface LastYozmPMNews {
  title: string;
}

const yozmPMJSONFile = "./yozm-it-news-pm.json";

const getLastNewsTitle = (): string | null => {
  const json = getJSON<LastYozmPMNews>(yozmPMJSONFile);
  return json?.title ?? null;
};

const setLastNewsTitle = (news: LastYozmPMNews) => {
  setJSON(yozmPMJSONFile, news);
};
...
const getPMYozmList = async () => {
  ...
  try {
    html = await axios.get(yozmBaseURL + "/magazine/list/plan");
...
export const sendPMYozm = async () => {
  ...
  await webhookClient.send({
    content: `## 오늘의 요즘IT-기획 최신글 ${filteredList.length}개!`,
    embeds: embeds
  });
